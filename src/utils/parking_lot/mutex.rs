...
use crate::utils::parking_lot::{deadlock, DEFAULT_PARK_TOKEN, parking_lot_core, ParkResult, UnparkResult, UnparkToken, util};
...
unsafe impl lock_api::RawMutexTimed for RawMutex {
    ...
    #[inline]
    fn try_lock_for(&self, timeout: core::time::Duration) -> bool {
        let result = if self
            .state
            .compare_exchange_weak(0, LOCKED_BIT, Ordering::Acquire, Ordering::Relaxed)
            .is_ok()
